...
- Never hand-write the `storybook`, `@rsbuild/core`, or `react-native-web` peer ranges in docs: write `<PeerRange name="storybook" />` and let the `replaceRules` in `rstack.config.ts` expand it from `packages/*/package.json` at build time (HTML, `.md`, and llms.txt alike). A name missing from that map fails the build as an undefined MDX component; add it to `peerRanges` there. Lower bounds for other peers (`react`, `vue`, `lit`) stay hand-written.
