...
- **≤ 2.1.x**: Configure options through `framework.options.builder.addonDocs` (see snippet below).
- **≥ 2.2.0**: Prefer configuring `@storybook/addon-docs` directly in the `addons` array (see second snippet). This mirrors Storybook's native API and matches the sandboxes.
- **3.0.0**: The old `addonDocs` builder option will be removed. Migrate to the `addons`-array syntax to stay compatible.
:::
...
```ts
// .storybook/main.ts (Storybook Rsbuild ≥ 2.2.0)
import remarkGfm from 'remark-gfm'
