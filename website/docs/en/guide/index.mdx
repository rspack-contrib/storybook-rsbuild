...
- **`@rsbuild/core` <PeerRange name="@rsbuild/core" />** — Storybook Rsbuild depends on the stable Rsbuild 1.0 runtime. See the [1.0 release announcement](https://rsbuild.rs/community/releases/v1-0) for background.
- **`storybook` <PeerRange name="storybook" />** — The range mirrors the `storybook` peer of the official Storybook renderer packages and moves forward with each Storybook release; the value shown is read from the package manifests at build time. Upgrade guidance is available in the [Storybook release notes](https://storybook.js.org/blog/storybook-10/).
